...
for i = 1 : T
    ...
    if flag_hardnegative
        ...
        [z_optimal, i_optimal, h_optimal] = compute_errors_realBoost(Y, features_train, w, B);
    end
    
    
    responses = compute_response_realBoost(h_optimal, i_optimal, features_train, B);
    w = w .* exp(-Y .* responses) ./ z_optimal;
    ...
    features_train(i_optimal, :) = 200;
    
    
    [verify_scores, verify_Y] = strong_classifier_verify_realBoost(h_H, index_H, features, i, B);
...
title('Training Error of Strong Classifier (RealBoost)', 'FontSize', 20);
grid on
...
histogram(training_scores(N_pos+1:end, 1), 100, 'FaceColor', 'r');
hold off
...
histogram(training_scores(N_pos+1:end, 2), 100, 'FaceColor', 'r');
hold off
legend('Pos (face)', 'Neg (non-face)');
xlabel('Scores', 'FontSize', 20);
ylabel('Number', 'FontSize', 20);
...
figure(6)
hold on
histogram(training_scores(1:N_pos, 3), 20, 'FaceColor', 'b');
histogram(training_scores(N_pos+1:end, 3), 400, 'FaceColor', 'r');
hold off
legend('Pos (face)', 'Neg (non-face)');
xlabel('Scores', 'FontSize', 20);
ylabel('Number', 'FontSize', 20);
...
figure(7)
hold on
histogram(training_scores(1:N_pos, 4), 40, 'FaceColor', 'b');
histogram(training_scores(N_pos+1:end, 4), 800, 'FaceColor', 'r');
hold off
legend('Pos (face)', 'Neg (non-face)');
xlabel('Scores', 'FontSize', 20);
ylabel('Number', 'FontSize', 20);
